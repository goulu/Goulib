...
class SortedCollection(list):
    ...
    def __reduce__(self):
        return self.__class__, (self, self._given_key)

    def __contains__(self, item):
        k = self._key(item)
        i = bisect_left(self._keys, k)
        j = bisect_right(self._keys, k)
        return item in self[i:j]
    ...
    def findi(self, k):
        'Return index of first item with a key == k.  Raise KeyError if not found.'
        i = bisect_left(self._keys, k)
        if i != len(self) and self._keys[i] == k:
            return i
        raise KeyError('No item found with key equal to: %r' % (k,))
    
    def find(self, k):
        return self[self.findi(k)]
